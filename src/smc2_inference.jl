...
function pmcmc_mean_cov(param_matrix::Array{Float64, 2}, log_w::Vector{Float64})
    ...
    for n in 1:size(param_matrix)[2]
...
function trunc_normal_jump(x::Vector, mu::Vector, sigma::Matrix;
                           lower_bound = (x, n) -> -Inf,
                           upper_bound = (x, n) -> Inf,
                           perturb=1e-4)
...
function trunc_normal_dens(x::Vector, x2::Vector, mu::Vector, sigma::Matrix;
                           lower_bound = (x, n) -> -Inf,
                           upper_bound = (x, n) -> Inf,
                           perturb=1e-4)
    sigma = sigma + I*perturb
...
function smc_pmcmc_proposal(theta_current::Vector, pmcmc_mean::Vector, pmcmc_cov::Array;
                            C = (2.38^2) / length(theta_current), kwargs...)
    return trunc_normal_jump(theta_current, theta_current, pmcmc_cov * C; kwargs...)
end

function smc_pmcmc_proposal_logdens(theta_current::Vector, theta_proposed::Vector, pmcmc_mean::Vector, pmcmc_cov::Matrix;
                                    C = (2.38^2) / length(theta_current), kwargs...)
    return trunc_normal_dens(theta_current, theta_proposed, theta_current, pmcmc_cov * C; kwargs...)
end
...
function density_tempered_pmcmc(thetas, logliks, loglik_fun, prior_fun, ξ, ξ_diff; pmcmc_theta_steps=10, rprop_pmh = smc_pmcmc_proposal, dprop_pmh = smc_pmcmc_proposal_logdens, kwargs...)
    ...
    for step_idx in 1:pmcmc_theta_steps
        starttime = Dates.now()
        res    = pmap(pmcmc_propose_accept_reject, thetas, logliks,
        cycle([loglik_fun]), cycle([prior_fun]), cycle([ξ]), cycle([pmcmc_mean]),
         cycle([pmcmc_cov]), cycle([rprop_pmh]), cycle([dprop_pmh]))
